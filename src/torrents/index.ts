...
export default class Transmission {
    ...
    const getUrl = () => {
      request.get(this.uri, this.options, (err, response) => {
        ...
        if (response.statusCode < 300 || response.statusCode > 399)
          return callback(
            new Error(`StatusCode=${response.statusCode} not equal 3XX`)
          );
  ...
  getParam(
    ...
    cycle: number = 0,
    isHashArray: boolean = true,
    isParamsArray: boolean = true
  ): void {
    if (!(hash instanceof Array)) {
      hash = [hash];
      isHashArray = false;
    } else {
      if (!hash.length) return callback(new Error("hash can't defined"));
    }
    ...
    if (!(param instanceof Array)) {
      ...
      if (!param.length) return callback(new Error("param can't defined"));
    }
    ...
    request.post(this.uri, opts, (err, response, body) => {
      ...
      if (
        ...
        if (cycle > 1)
          return callback(
            new Error('Error looping with x-transmission-session-id')
          );
      ...
      if (body.result && body.result === 'success' && body.arguments) {
        ...
        if (!args || args.length !== hash.length)
          return callback(
            new Error("Response args count can't  equal requested hashes")
          );
        ...
        for (let i = 0; i < args.length; i++) {
          for (let j = 0; j < param.length; j++)
            if (typeof args[i][param[j]] === 'undefined')
              return callback(
                new Error("Can't find requested param " + param[j])
              );

          if (!isParamsArray) args[i] = args[i][param[0]];
        }

        if (isHashArray) callback(null, args);
        else callback(null, args[0]);
      } else callback(new Error("Can't find torrent data"), body);
    });
  }

  freeSpace(
    path: string,
    callback: (err?: Error | null, space?: {}, body?: {}) => void,
    cycle: number = 0
  ): void {
    const opts = _.cloneDeep(this.options);
    opts.body = {
    ...
    request.post(this.uri, opts, (err, response, body) => {
      ...
      if (body.result && body.result === 'success') {
        if (!body.arguments || !(body.arguments instanceof Object))
          return callback(
            new Error("Can't parse arguments: " + body.arguments)
          );

        callback(null, body.arguments);
      } else callback(new Error("Can't calculate free space"), body);
    });
  }
  ...
  addTorrent(
    ...
    request.post(this.uri, opts, (err, response, body) => {
      ...
      if (response.statusCode !== 200)
        return callback(
          new Error(
            `Bad statusCode = ${response.statusCode} for torrent ${torrentFileName}`
          )
        );
      ...
      if (body.result && body.result === 'success') {
        if (body.arguments['torrent-added']) {
          ...
          callback(
            new Error(
              `Torrent ${torrentFileName} does not added, reason: ${JSON.stringify(
                body
              )}`
            )
          );
  ...
  removeTorrent(
    hash: string | Array<string>,
    deleteData: boolean,
    callback: (err?: Error | null, body?: {}) => void,
    cycle: number = 0,
    isHashArray: boolean = true
  ): void {
    if (!(hash instanceof Array)) {
    ...
    request.post(this.uri, opts, (err, response, body) => {
      ...
      if (body.result && body.result === 'success') {
        callback();
      } else callback(new Error('Torrent does not removed'), body);
    });
  }
  ...
  stopTorrent(
    ...
    request.post(this.uri, opts, (err, response, body) => {
      ...
      if (body.result && body.result === 'success') {
        ...
        callback(
          new Error(
            `Method ${action ? 'torrent-start' : 'torrent-stop'} can't applied`
          ),
          body
        );
    });
