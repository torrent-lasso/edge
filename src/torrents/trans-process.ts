...
export default class TransProcess {
    ...
    constructor(path: string, args: Array<string>, user?: string, password?: string) {
    ...
    init(callback: (err?:Error|null)=>void):void{
        __editConfig(this.args[2], this.user, this.password, (err) => {
            ...
            setTimeout(()=>{if (!hasErr) callback();}, 2000);
        });
    }
...
function __editConfig(configPath: string, user: string|undefined, password: string|undefined, callback: (err?:Error|null)=>void):void {
    fs.readFile(path.join(configPath, 'settings.json'), 'utf8', (err, data)=>{
       ...
       fs.writeFile(path.join(configPath, 'settings.json'), JSON.stringify(config), callback);
    });
}