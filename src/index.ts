...
const proto: IProto =
  Config.get('transmission:proto') ||
  Config.get('transmission_proto') ||
  'http';
log.debug(`transmission:proto=${proto}`);
const host: string =
  Config.get('transmission:host') ||
  Config.get('transmission_host') ||
  'localhost';
log.debug(`transmission:host=${host}`);
const port: number =
  Config.get('transmission:port') || Config.get('transmission_port') || 9091;
log.debug(`transmission:port=${port}`);
const user: string | undefined =
  Config.get('transmission:user') ||
  Config.get('transmission_user') ||
  undefined;
log.debug(`transmission:user=${user}`);
const password: string | undefined =
  Config.get('transmission:password') ||
  Config.get('transmission_password') ||
  undefined;
log.debug(`transmission:password=${password}`);
const path: string | undefined =
  Config.get('transmission:path') ||
  Config.get('transmission_path') ||
  undefined;
log.debug(`transmission:path=${path}`);
const args: Array<string> | undefined =
  Config.get('transmission:args') ||
  Config.get('transmission_args') ||
  undefined;
log.debug(`transmission:args=${args}`);

const token: string =
  Config.get('torrentlassobot:token') || Config.get('torrentlassobot_token');
log.debug(`torrentlassobot:token=${token}`);

const uri: string =
  _.trimEnd(
    Config.get('torrentlassobot_uri') || Config.get('torrentlassobot:uri'),
    '/'
  ) +
  '/?token=' +
  token;
...
torrent.init((err) => {
  ...
  socket.on(
    ...
    (
      ...
      callback: (
        err?: string | null,
        status?: Object | Array<Object> | string | number
      ) => void
    ) => {
      ...
      torrent.getParam(hash, param, (err, status) => {
        ...
        log.debug(
          `torrent-get with hash=${hash}, param=${param} returns status=${status}`
        );
  ...
  socket.on(
    'free-space',
    (path: string, callback: (err?: string | null, space?: {}) => void) => {
      ...
      torrent.freeSpace(path, (err, space) => {
        ...
        log.debug(
          `free-space with path=${path} returns space=${JSON.stringify(space)}`
        );
        callback(null, space);
      });
    }
  ...
  socket.on(
    ...
    (
      ...
      log.debug(
        `Received message="get-param" for hash=${hash}, param=${param}`
      );
      ...
      torrent.getParam(hash, param, (err, status) => {
        ...
        log.debug(
          `get-param with hash=${hash}, param=${param} returns status=${JSON.stringify(
            status
          )}`
        );
        callback(null, status);
  ...
  socket.on(
    ...
    (
      ...
      log.debug(
        `Received message="torrent-add" for torrentFileName=${torrentFileName}, torrentData.length=${torrentData.length}, dir=${dir}`
      );
      ...
      torrent.addTorrent(
        ...
        torrentData,
        dir,
        (err, status, torrentHash, id) => {
          if (err) return callback(err.message);

          log.debug(
            `torrent-add with torrentFileName=${torrentFileName}, torrentData.length=${torrentData.length}, dir=${dir} returns status=${status}, torrentHash=${torrentHash}, id=${id}`
          );
  ...
  socket.on(
    ...
    (
      ...
      log.debug(
        `Received message="torrent-remove" for hash=${hash}, deleteData=${deleteData}`
      );
      ...
      torrent.removeTorrent(hash, deleteData, (err, body) => {
        ...
        log.debug(
          `torrent-remove with hash=${hash}, deleteData=${deleteData} returns OK!`
        );
        callback();
      });
    }
  ...
  socket.on(
    ...
    (
      ...
      log.debug(
        `Received message="torrent-stop" for hash=${hash}, action=${action}`
      );
      ...
      torrent.stopTorrent(hash, action, (err, body) => {
        ...
        log.debug(
          `torrent-stop with hash=${hash}, action=${action} returns OK!`
        );
        callback();
      });
    }
  ...
  socket.on('start-transfer', (filename: string, transferId: number) => {
    ...
    readStream.on('data', (chunk) => {
      ...
      socket.emit(
        'file-transfer',
        transferId,
        Buffer.from(chunk, 'binary').toString('base64')
      );
    });
